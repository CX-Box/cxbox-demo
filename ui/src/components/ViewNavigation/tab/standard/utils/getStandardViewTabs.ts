...
const getNodePath = (parentPath: string | undefined, uniqueValue: string) =>
    parentPath ? [parentPath, uniqueValue].join(PATH_SEPARATOR) : uniqueValue

const getArrayFromNodePath = (path?: string) => path?.split(PATH_SEPARATOR)

const isViewNavigationItem = (item: object): item is { viewName?: unknown } => !!item && 'viewName' in item

const hasTitle = (item: object): item is { title?: unknown } => !!item && 'title' in item
