...
export const useImageControl = (imageRef: React.RefObject<HTMLImageElement>, enabled: boolean = false) => {
    ...
    const { scale, increaseScale, decreaseScale, rollback: rollbackScale } = useImageScale(imageRef)
...
const useImageRotate = () => {
    const [rotate, setRotate] = useState(DEFAULT_ROTATE)

    const rotateClockwise = useCallback(() => {
        setRotate(prevState => {
            let newState = prevState + 90
            newState = newState >= 360 ? 0 : newState
    ...
    const rotateCounterClockwise = useCallback(() => {
        setRotate(prevState => {
            let newState = prevState - 90
            newState = newState < 0 ? 270 : newState
...
const useImageScale = (imageRef: React.RefObject<HTMLImageElement>) => {
