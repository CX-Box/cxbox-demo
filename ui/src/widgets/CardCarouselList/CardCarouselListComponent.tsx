...
import FilePreviewCard from '@widgets/CardCarouselList/components/FilePreviewCard'
import { usePrevious } from '@hooks/usePrevious'
import { actions, DictionaryFieldMeta } from '@cxbox-ui/core'
import HorizontalScrollContainer from '@widgets/CardCarouselList/components/HorizontalScrollContainer'
import Pagination from '@components/ui/Pagination/Pagination'
import WidgetTitle from '@components/WidgetTitle/WidgetTitle'
import InnerReadWidget from '@widgets/CardCarouselList/components/InnerReadWidget'
...
interface CardCarouselListProps extends BaseWidgetProps {
    type?: 'carousel' | 'list'
}
...
function assertIsCardCarouselListMeta(meta: BaseWidgetProps['widgetMeta']): asserts meta is CardCarouselListWidgetMeta {
    if (meta.type !== 'CardCarouselList') {
        throw new Error('Not a CardCarouselList meta')
    }
}

const CardCarouselListComponent: WidgetComponentType<CardCarouselListProps> = ({ widgetMeta, type = 'carousel' }) => {
    assertIsCardCarouselListMeta(widgetMeta)
    const isCarousel = type === 'carousel'
    const { t } = useTranslation()
    const { bcName, name: widgetName } = widgetMeta
    const cardOptions = widgetMeta.options?.card

    const data = useAppSelector(state => selectBcData(state, bcName))
    const totalCount = data?.length ?? 0

    const widgetField = useMemo(() => {
        const fields = widgetMeta?.fields as FileUploadFieldMeta[] | undefined
        return fields?.find(field =>
            cardOptions?.valueFieldKey ? cardOptions.valueFieldKey === field.key : field.type === FieldType.fileUpload
        )
    }, [cardOptions?.valueFieldKey, widgetMeta?.fields])

    const statusField = useMemo(() => {
        const fields = widgetMeta?.fields as DictionaryFieldMeta[] | undefined
        return fields?.find(field => field.type === FieldType.dictionary)
    }, [widgetMeta?.fields])

    const paginationProps = useArrowPagination(widgetMeta)
    ...
    const mainFileElement = useMemo(() => {
        if (!data?.length || !widgetMeta.options?.read?.widget) {
            return null
        }

        return (
            <div className={cn(styles.main)}>
                <InnerReadWidget widgetName={widgetMeta.options.read.widget} />
            </div>
        )
    }, [data?.length, widgetMeta.options?.read?.widget])
            ...
            >
                {statusField ? (
                    <div className={styles.status}>
                        {renderTitle(widgetMeta, `$\{${statusField.key}\}`, dataItem.id, PREVIEW_STATUS_OPACITY)}
                    </div>
                ) : null}
            </FilePreviewCard>
        )
    ...
    return (
        <div className={cn(styles.root)}>
            {mainFileElement}
            {previewListElement}
            {!isCarousel && <Pagination meta={widgetMeta} />}
        </div>
    )
}

export default React.memo(CardCarouselListComponent)
